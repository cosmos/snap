...
export class AddressState {
    ...
    public static async getAddress(chain_id : string){
        ...
        if (addressList.length == 0) {
            throw new Error(`${chain_id} is not found. Add the address to your address book at https://wallet.mysticlabs.xyz`)
        }
     ...
     */
        public static async addAddress(address: Address) {
            ...
            await snap.request({
                method: 'snap_manageState',
                params: { operation: 'update', newState: { addresses: addresses.string() } },
            });
        }
     ...
     */
       public static async addAddresses(addresses: Addresses): Promise<Boolean> {
        ...
        await snap.request({
            method: 'snap_manageState',
            params: { operation: 'update', newState: { ...data, addresses: addresses.string() } },
        });
